...
def convert_file(file_path, temp_path, entity_dict, subset={}, add_id="", entity_name="", asko_client=None, paths=[], sheet=0):
...
def main():
    ...
    for pattern, validation_files in patterns.items():
        ...
        for path, file in data.items():
            ...
            res, entity_dict = convert_file(os.path.join(path, filename), os.path.join("tmp/", new_file_name), entity_dict, subset=subset, paths=paths, add_id=new_id, entity_name=entity_name, asko_client=asko_client, sheet=sheet)
