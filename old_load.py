...
def convert_file(file_path, temp_path, entity_dict, subset={}, add_id="", entity_name="", asko_client=None, gopublic_data=None, paths=[], sheet=0):
    ...
    for path in paths:
        ...
        if path.get("generate_uri"):
            base_path = path.get("base_path")
            df[path["col"]] = df[path["col"]].apply(lambda x: get_file_uri(gopublic_data, base_path, x))
...
def get_file_uri(gopublic_data, base_path, file_path):
    gopublic_client = gopublic_data['client']
    token = gopublic_data['token']
    base_url = gopublic_data['base_url']
